...
In other words, the point $P$ with vector $\vec{p}$ is on this line if and only if a real number $t$ exists such that
\begin{align*}
...
Consider now points in $\mathbb{R}^3$. The line through $P_0 = (x_0 ,y_0 ,z_0 )$ with direction vector $\vec{d}=\begin{bmatrix} a & b & c \end{bmatrix}^T\neq0$ is given by
\begin{align*}
...
An equation of a line is **not unique**.    
</div>
